...
export interface IDashboardProps {
  ...
  onShowChannelTemplate: Function;
...
class Dashboard extends React.PureComponent<IDashboardProps, IDashboardState> {
  ...
  getDashboardData = async () => {
    ...
    try {
      ...
      const allIncidents = this.sortDashboardData(
        await this.dataService.getDashboardData(graphEndpoint, this.props.graph)
      );
      ...
      this.props.showMessageBar(
        this.props.localeStrings.genericErrorMessage +
          ", " +
          this.props.localeStrings.getIncidentsFailedErrMsg,
      ...
      this.dataService.trackException(
        ...
        error,
        constants.componentNames.DashboardComponent,
        "GetDashboardData",
  ...
  sortDashboardData = (allIncidents: any): any => {
    return allIncidents.sort((currIncident: any, prevIncident: any) =>
      parseInt(currIncident.itemId) < parseInt(prevIncident.itemId)
        ? 1
        : parseInt(prevIncident.itemId) < parseInt(currIncident.itemId)
        ? -1
        : 0
    );
  };
  ...
  statusIcon = (cell: any, row: any, rowIndex: any, formatExtraData: any) => {
    if (row.status === constants.closed) {
      return (
        <img
          src={require("../assets/Images/ClosedIcon.svg").default}
          alt={this.props.localeStrings.closed}
    ...
    if (row.status === constants.active) {
      return (
        <img
          src={require("../assets/Images/ActiveIcon.svg").default}
          alt={this.props.localeStrings.active}
          className="status-icon"
        />
      );
    }
    if (row.status === constants.planning) {
      return (
        <img
          src={require("../assets/Images/PlanningIcon.svg").default}
          alt={this.props.localeStrings.planning}
          className="status-icon"
        />
      );
    }
  ...
  _customRenderer(
    ...
    return (
      <span>
        <img
          src={require(`../assets/Images/${link.itemKey}ItemsSelected.svg`)}
          alt={`${link.headerText}`}
          id="pivot-icon-selected"
        />
        <img
          src={require(`../assets/Images/${link.itemKey}Items.svg`)}
          alt={`${link.headerText}`}
          id="pivot-icon"
        />
  ...
  searchDashboard = (searchText: any) => {
    ...
    let filteredAllIncidents = allIncidents.filter((incident: any) => {
      return (
        (incident["incidentName"] &&
          incident["incidentName"].toLowerCase().indexOf(searchKeyword) > -1) ||
        (incident["incidentId"] &&
          incident["incidentId"].toString().toLowerCase().indexOf(searchKeyword) > -1) ||
        (incident["incidentCommander"] &&
          incident["incidentCommander"].toLowerCase().indexOf(searchKeyword) > -1) ||
    ...
    this.setState({
      ...
      filteredPlanningIncidents: filteredAllIncidents.filter(
        (e: any) => e.status === constants.planning
      ),
      filteredActiveIncidents: filteredAllIncidents.filter(
        (e: any) => e.status === constants.active
      ),
      filteredCompletedIncidents: filteredAllIncidents.filter(
        (e: any) => e.status === constants.closed
      ),
    });
  };
  ...
  teamsDeepLink = (cell: any, gridRow: any, rowIndex: any, formatExtraData: any) => {
    return (
      <span className="incIdDeepLink" onClick={() => this.onDeepLinkClick(gridRow)}>
        {gridRow.incidentId}
      </span>
    );
  };

  spoSiteLink = (cell: any, gridRow: any, rowIndex: any, formatExtraData: any) => {
    return (
      <a
        href={`${siteConfig.spoSiteBaseUrl}/${constants.teamEOCPrefix}_${gridRow.incidentId}`}
        title={this.props.localeStrings.openSPOSite}
        target="_blank"
        rel="noreferrer"
      >
        {gridRow.incidentName}
      </a>
      // <span className='incIdDeepLink' onClick={() => this.onDeepLinkClick(gridRow)}>
      //   {gridRow.incidentId}
      // </span>
          ...
          <div>
                  ...
                  <Flex space="between" wrap={true}>
                    <div
                      className={`manage-links${
                        this.state.isManageCalloutVisible ? " callout-visible" : ""
                      }`}
                      onClick={() =>
                        this.setState({
                          isManageCalloutVisible: !this.state.isManageCalloutVisible,
                        })
                      }
                      id={manageButtonId}
                    >
                  ...
                  >
                    <div>
                      ...
                      <div
                        ...
                        title={this.props.localeStrings.channelTemplateTooltip}
                        onClick={() => this.props.onShowChannelTemplate()}
                      >
                        <span className="team-name-link">
                          <img
                            src={require("../assets/Images/ListViewIcon.svg").default}
                            alt={this.props.localeStrings.channelTemplateTooltip}
                          />
                          <span className="manage-callout-text">
                            {this.props.localeStrings.channelTemplateLabel}
                          </span>
                        </span>
                      </div>
                      <div className="dashboard-link">
                        <a
                          ...
                          href={`https://${this.props.tenantName}/sites/${this.props.siteName}/lists/${siteConfig.incTypeList}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          <img
                            src={require("../assets/Images/Manage Incident Types.svg").default}
                            alt={this.props.localeStrings.manageIncidentTypesTooltip}
                          />
                          <span className="manage-callout-text">
                            {this.props.localeStrings.incidentTypesLabel}
                          </span>
                        </a>
                      </div>
                      <div className="dashboard-link">
                        <a
                          title={this.props.localeStrings.manageRolesTooltip}
                          href={`https://${this.props.tenantName}/sites/${this.props.siteName}/lists/${siteConfig.roleAssignmentList}`}
                          target="_blank"
                          rel="noreferrer"
                        >
                          <img
                            src={require("../assets/Images/Manage Roles.svg").default}
                            alt={this.props.localeStrings.manageRolesTooltip}
                          />
                          <span className="manage-callout-text">
                            {this.props.localeStrings.roles}
                          </span>
                        </a>
                      </div>
                      <div
                      ...
                      >
                        <span className="team-name-link">
                          <img
                            ...
                            alt={this.props.localeStrings.manageTeamNameTooltip}
                          />
                          <span className="manage-callout-text">
                            {this.props.localeStrings.teamNameLabel}
                          </span>
                        </span>
                      </div>
                    </div>
            ...
            <div id="dashboard-pivot-container">
              <div className="container">
                ...
                <Flex
                  gap={this.state.isDesktop ? "gap.medium" : "gap.small"}
                  space="evenly"
                ...
                >
                  ...
                  <Flex gap={this.state.isDesktop ? "gap.small" : "gap.smaller"}>
                    <img
                      src={require("../assets/Images/PlanningItems.svg").default}
                      alt="Planning Items"
                    />
                  ...
                  <Flex gap={this.state.isDesktop ? "gap.small" : "gap.smaller"}>
                    <img
                      src={require("../assets/Images/ActiveItems.svg").default}
                      alt="Active Items"
                    />
                  ...
                  <Flex gap={this.state.isDesktop ? "gap.small" : "gap.smaller"}>
                    <img
                      src={require("../assets/Images/ClosedItems.svg").default}
                      alt="Completed Items"
                    />
                ...
                >
                  ...
                  >
                    <BootstrapTable
                      ...
                      noDataIndication={() => (
                        <div>{this.props.localeStrings.noIncidentsFound}</div>
                      )}
                    />
                  </PivotItem>
                  <PivotItem
