...
export class TeamNameConfig extends React.PureComponent<
  ITeamNameConfigProps,
  ITeamNameConfigState
> {
  ...
  getTeamNameConfigData = async () => {
    try {
      ...
      this.setState({
        ...
        showLoader: false,
        prefixIsMissing:
          configData.value[constants.teamNameConfigConstants.Prefix] !==
            constants.teamNameConfigConstants.DontInclude &&
          configData.value[constants.teamNameConfigConstants.Prefix] === ""
            ? true
            : false,
      });
      ...
      this.dataService.trackException(
        ...
        `${teamNameConfigString}_GetConfiguration`,
  ...
  updateConfiguration = async () => {
    ...
    if (this.state.prefixIsMissing) {
      ...
      try {
        ...
        const configUpdated = await this.dataService.updateTeamNameConfigData(
          graphEndpoint,
          this.props.graph,
          updatedValues
        );
        if (configUpdated) {
          ...
          this.props.showMessageBar(
            this.props.localeStrings.updatedConfigSuccessMessage,
          ...
          this.dataService.trackTrace(
            this.props.appInsights,
            "Team Name Configurations Updated",
            "",
            this.props.userPrincipalName
          );
          ...
          this.props.showMessageBar(
            this.props.localeStrings.genericErrorMessage +
              ", " +
              this.props.localeStrings.updatedConfigErrorMessage,
            constants.messageBarType.error
          );
          ...
          this.dataService.trackTrace(
            this.props.appInsights,
            "Team Name Configurations Update Failed",
            "",
            this.props.userPrincipalName
          );
        }
        ...
        this.dataService.trackException(
          ...
          error,
          constants.componentNames.IncidentDetailsComponent,
          `${teamNameConfigString}_UpdateConfiguration`,
          this.props.userPrincipalName
        );
      }
    }
  ...
  updateOrder = (selectedVal: any, configData: any, key: string) => {
    try {
      ...
      let minVal =
        parseInt(configData[key]) > parseInt(selectedVal.value)
          ? parseInt(selectedVal.value)
          : parseInt(configData[key]);
      let maxVal =
        parseInt(configData[key]) > parseInt(selectedVal.value)
          ? parseInt(configData[key])
          : parseInt(selectedVal.value);
      let isDuplicateValuePresent = Object.values(filteredArr).includes(
        parseInt(selectedVal.value)
      );
      filteredArr[key] =
        selectedVal.value === constants.teamNameConfigConstants.DontInclude
          ? selectedVal.value
          : parseInt(selectedVal.value);
      ...
      Object.keys(filteredArr).forEach((k: any) => {
              ...
              if (filteredArr[k] !== constants.teamNameConfigConstants.DontInclude) {
                if (selectedCount === 2) {
                  if (parseInt(selectedVal.value) === 1)
                    filteredArr[k] = parseInt(filteredArr[k]) + 1;
                  else filteredArr[k] = parseInt(filteredArr[k]) - 1;
      ...
      this.setState({
        ...
        prefixIsMissing:
          filteredArr[constants.teamNameConfigConstants.Prefix] !==
            constants.teamNameConfigConstants.DontInclude &&
          this.state.configValue.PrefixValue === ""
            ? true
            : false,
      });
      ...
      this.dataService.trackException(
        this.props.appInsights,
        error,
        constants.componentNames.TeamNameConfiguration,
        `${teamNameConfigString}_UpdateOrder`,
        this.props.userPrincipalName
      );
    }
  };
  ...
  formatPreviewString = (configData: any) => {
    try {
      ...
      Object.keys(configData).forEach((key) => {
        if (configData[key] !== constants.teamNameConfigConstants.DontInclude) {
          if (key === constants.teamNameConfigConstants.Prefix)
            string = string ? string + " - Prefix" : "Prefix";
          if (key === constants.teamNameConfigConstants.IncidentName)
            string = string ? string + " - Incident Name" : "Incident Name";
          if (key === constants.teamNameConfigConstants.IncidentType)
            string = string ? string + " - Incident Type" : "Incident Type";
          if (key === constants.teamNameConfigConstants.StartDate)
            string = string ? string + " - Start Date" : "Start Date";
        }
      });
      ...
      this.dataService.trackException(
        ...
        error,
        constants.componentNames.TeamNameConfiguration,
        `${teamNameConfigString}_FormatPreviewString`,
        this.props.userPrincipalName
      );
    }
  };
              ...
              <div className="team-name-config-form-area">
                <div className="container">
                      ...
                      <Col xl={5} lg={6} md={7} sm={12} xs={12}>
                        <Row xl={2} lg={2} md={2} sm={2} xs={2}>
                            ...
                            <FormDropdown
                              ...
                              onChange={(evt, val) =>
                                this.updateOrder(
                                  val,
                                  this.state.configValue,
                                  constants.teamNameConfigConstants.Prefix
                                )
                              }
                            />
                          </Col>
                          <Col xl={7} lg={7} md={5} sm={7} xs={7}>
                            ...
                            />
                            {this.state.prefixIsMissing ? (
                              <label className="message-label">
                                {this.props.localeStrings.prefixValueRequired}
                              </label>
                            ) : (
                              <></>
                            )}
                            ...
                            <FormDropdown
                              ...
                              onChange={(evt, val) =>
                                this.updateOrder(
                                  val,
                                  this.state.configValue,
                                  constants.teamNameConfigConstants.IncidentName
                                )
                              }
                            />
                          </Col>
                          <Col xl={7} lg={7} md={5} sm={7} xs={7}>
                            <label className="team-name-config-dropdown-label">
                              {this.props.localeStrings.incidentNameLabel}
                            </label>
                          </Col>
                        </Row>
                        <Row xl={2} lg={2} md={2} sm={2} xs={2}>
                            ...
                            <FormDropdown
                              ...
                              onChange={(evt, val) =>
                                this.updateOrder(
                                  val,
                                  this.state.configValue,
                                  constants.teamNameConfigConstants.IncidentType
                                )
                              }
                            />
                          </Col>
                          <Col xl={7} lg={7} md={5} sm={7} xs={7}>
                            <label className="team-name-config-dropdown-label">
                              {this.props.localeStrings.incidentTypeLabel}
                            </label>
                          </Col>
                        </Row>
                        <Row xl={2} lg={2} md={2} sm={2} xs={2}>
                            ...
                            <FormDropdown
                              ...
                              onChange={(evt, val) =>
                                this.updateOrder(
                                  val,
                                  this.state.configValue,
                                  constants.teamNameConfigConstants.StartDate
                                )
                              }
                            />
                          </Col>
                          <Col xl={7} lg={7} md={5} sm={7} xs={7}>
                            <label className="team-name-config-dropdown-label">
                              {this.props.localeStrings.startDate}
                            </label>
                          </Col>
                        </Row>
                      </Col>
                      <Col xl={7} lg={6} md={5} sm={12} xs={12}>
                        <div className="team-name-config-preview-heading">
                        ...
                        <Row xl={2} lg={2} md={2} sm={2} xs={2}>
                          ...
                          <Col
                            ...
                            sm={10}
                            xs={12}
                            className="team-name-config-order-preview-area"
                          >
                            <div
                  ...
                  <div className="team-name-config-btn-area">
                    ...
                    >
                      <label className="team-name-config-back-btn-label">
                        {this.props.localeStrings.btnBack}
                      </label>
                    </Button>
                    <Button
                      primary
                    ...
                    >
                      <label className="team-name-config-save-btn-label">
                        {this.props.localeStrings.btnSaveChanges}
                      </label>
                    </Button>
                  </div>
                </div>
