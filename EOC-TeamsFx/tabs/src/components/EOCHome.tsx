...
let siteName = process.env.REACT_APP_SHAREPOINT_SITE_NAME?.toString().replace(
  /\s+/g,
  ""
);
...
export class EOCHome extends React.Component<IEOCHomeProps, IEOCHomeState> {
  ...
  public loginClick = async () => {
    ...
    const profile = await this.dataService.getGraphData(
      graphConfig.meGraphEndpoint,
      this.state.graph
    ); // get user profile to validate the API
  ...
  public async getTenantAndSiteDetails() {
    try {
      // get the tenant name
      const tenantName = await this.dataService.getTenantDetails(
        graphConfig.rootSiteGraphEndpoint,
        this.state.graph
      );
  ...
  public async getCurrentUserDetails() {
    try {
      // get the tenant name
      const currentUser = await this.dataService.getGraphData(
        graphConfig.meGraphEndpoint,
        this.state.graph
      );
      this.setState({
  ...
  private showEditForm = async (incidentData: any) => {
    ...
    try {
      ...
      const teamGroupId = incidentData.teamWebURL
        .split("?")[1]
        .split("&")[0]
        .split("=")[1]
        .trim();
  ...
  private checkIfUserHasPermissionToEdit = async (
    teamId: string
  ): Promise<any> => {
    let isOwner = false;
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const existingMembers = await this.dataService.getExistingTeamMembers(
          graphEndpoint,
          this.state.graph
        );
        ...
        existingMembers.value.forEach((members: any) => {
          if (
            ...
            members.userId === this.state.currentUserId
          ) {
            isOwner = true;
          }
        });
  ...
  // changes state to show channel template setting
  private onShowChannelTemplate = () => {
    this.setState({ showChannelTemplate: true });
    this.hideMessageBar();
  };
          ...
          <>
            ...
            {this.state.showLoginPage && (
              <div className="loginButton">
                <Button
                  primary
                  content={localeStrings.btnLogin}
                  disabled={!this.state.showLoginPage}
            ...
            {!this.state.showLoginPage && this.state.siteId !== "" && (
              <div>
                ...
                {this.state.showLoader ? (
                  ...
                  />
                ) : this.state.showChannelTemplate ? (
                  <ChannelTemplate
                  ...
                  <>
                    {!this.state.showIncForm ? (
                      <Dashboard
                        ...
                        onShowChannelTemplate={this.onShowChannelTemplate}
                          ...
                          <>
                            ...
                            !this.state.showNoAccessMessage ? (
                              <IncidentDetails
                                ...
                                existingTeamMembers={
                                  this.state.existingTeamMembers
                                }
