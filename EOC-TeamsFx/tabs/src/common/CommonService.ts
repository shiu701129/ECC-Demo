...
import { ITemplateInfo } from "../components/ChannelTemplate";
...
export default class CommonService {
      ...
      incidentsData.value.forEach((item: any) => {
        formattedIncidentsData.push({
          ...
          startDateUTC: new Date(item.fields.StartDateTime)
            .toISOString()
            .slice(0, new Date(item.fields.StartDateTime).toISOString().length - 1),
          ...
          channelTemplate: item.fields.ChannelTemplate ? item.fields.ChannelTemplate : "",
        });
      });
  ...
  public async getDropdownOptions(graphEndpoint: any, graph: Client): Promise<any> {
    try {
      ...
      listData.value.forEach((item: any) => {
        drpdwnOptions.push(item.fields.Title);
      });
  ...
  public async getChannelTemplateList(graphEndpoint: any, graph: Client): Promise<any> {
    try {
      const listData = await graph.api(graphEndpoint).get();
      ...
      // Map the JSON response to the output array
      listData.value.forEach((item: any) => {
        result.push({ title: item.fields.Title, setting: JSON.parse(item.fields.Setting) });
      });
      return result;
    } catch (error) {
      console.error(
        constants.errorLogPrefix + "_CommonService_getChannelTemplateList \n",
        JSON.stringify(error)
      );
    }
  }

  public async getDescriptionList(graphEndpoint: any, graph: Client): Promise<any> {
    try {
      const listData = await graph.api(graphEndpoint).get();
      ...
      // Map the JSON response to the output array
      listData.value.forEach((item: any) => {
        result.push(item.fields.Title);
      });
      return result;
    } catch (error) {
      console.error(
        constants.errorLogPrefix + "_CommonService_getDescriptionList \n",
        JSON.stringify(error)
      );
    }
  }
  ...
  public async sendGraphPostRequest(
    graphEndpoint: any,
    graph: Client,
    requestObj: any
  ): Promise<any> {
  ...
  public async sendGraphPutRequest(
    graphEndpoint: any,
    graph: Client,
    requestObj: any
  ): Promise<any> {
  ...
  public async sendGraphPatchRequest(
    graphEndpoint: any,
    graph: Client,
    requestObj: any
  ): Promise<any> {
  ...
  public async updateTeamsDisplayName(
    graphEndpoint: any,
    graph: Client,
    requestObj: any
  ): Promise<any> {
  ...
  public async getIncidentTypeDefaultRolesData(graphEndpoint: any, graph: Client): Promise<any> {
    try {
      ...
      var formattedRolesData: Array<IIncidentTypeDefaultRoleItem> =
        new Array<IIncidentTypeDefaultRoleItem>();
  ...
  public regexValidation(incidentInfo: any): any {
    ...
    if (
      incidentInfo.incidentName.indexOf("#") > -1 ||
      incidentInfo.incidentName.indexOf("&") > -1
    ) {
      inputRegexValidationObj.incidentNameHasError = true;
    }
    if (incidentInfo.location !== undefined && incidentInfo.location !== "") {
      if (incidentInfo.location.indexOf("#") > -1 || incidentInfo.location.indexOf("&") > -1) {
        inputRegexValidationObj.incidentLocationHasError = true;
      }
    }
    if (
      inputRegexValidationObj.incidentLocationHasError ||
      inputRegexValidationObj.incidentNameHasError
    ) {
  ...
  public async updateTeamNameConfigData(
    graphEndpoint: any,
    graph: Client,
    listItemObj: any
  ): Promise<any> {
  ...
  public sortConfigData(configValues: Object) {
    try {
      const filteredObject = Object.fromEntries(
        Object.entries(configValues).filter(function ([key, value]) {
          return value > 0;
        })
      );
      ...
      incidentsData.value.forEach((item: any) => {
        ...
        if (item.id !== "2.0" && item.id !== "3.0") {
          formattedIncidentsData.push({
            ...
            startDate: this.formatDate(item.fields.StartDateTime),
            modifiedDate:
              new Date(item.fields.Modified).toDateString().slice(4) +
              " " +
              new Date(item.fields.Modified).toLocaleTimeString(),
            incidentDescription: item.fields.Description,
            incidentType: item.fields.IncidentType,
            roleAssignments: item.fields.RoleAssignment
              ? this.formatRoleAssignments(item.fields.RoleAssignment)
              : item.fields.RoleAssignment,
            roleAssignmentsObj: item.fields.RoleAssignment
              ? this.formatRoleAssignmentsForGrid(item.fields.RoleAssignment)
              : item.fields.RoleAssignment,
            severity: item.fields.Severity ? item.fields.Severity : "",
  ...
  public async getChannelTemplateData(graphEndpoint: any, graph: Client): Promise<any> {
    try {
      const data = await graph.api(graphEndpoint).get();
      var formattedData: Array<ITemplateInfo> = new Array<ITemplateInfo>();
      ...
      data.value.forEach((item: any) => {
        formattedData.push({
          ...
          setting: JSON.parse(item.fields.Setting),
          isAdd: false,
          isEdit: false,
          isDel: false,
        });
      ...
      console.error(
        constants.errorLogPrefix + "_CommonService_GetChannelTemplateData \n",
        JSON.stringify(error)
      );
    }
  }
